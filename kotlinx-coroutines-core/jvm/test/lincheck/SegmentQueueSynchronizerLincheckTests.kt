 ...
 * to make this implementation correct with prompt cancellation model,
...
internal class AsyncSemaphoreSmart(permits: Int) : SegmentQueueSynchronizer<Unit>(), Semaphore {
    ...
    override fun release() {
        ...
        // permit will be refused, so that this release
        // invocation can take effect with a small lag,
        // but it is guaranteed that it will take effect
        // eventually.
        resume(Unit)
    }
    ...
    override fun onCancellation(): Boolean {
        ...
        // is going to resume, or `false` if there is
        // one and this permit should be refused.
        return p < 0
    }
 ...
 * same time, [await] suspends until the count reaches zero.
 *
...
internal open class CountDownLatch(count: Int) : SegmentQueueSynchronizer<Unit>() {
    ...
    // so that the following suspension attempts detect this
    // via checking the mark and complete immediately.
 ...
 * [arrive] operation. The last coroutines should resume all the
...
internal class BlockingQueuePool<T: Any> : SegmentQueueSynchronizer<T>(), BlockingPool<T> {
    ...
    fun stateRepresentation(): String {
        ...
        val last = max(retrieveIdx.value, insertIdx.value)
...
/**
 * This algorithm maintains a separate queue for elements so that blocked [receive] operation
 * retrieves elements _after_ resumption from this queue. Similar to other data structures,
